...
8. **Deterministic kernels are the default.** `cecoppo.utils.set_seed` seeds Python, NumPy, and CPU/CUDA PyTorch streams and requests deterministic PyTorch algorithms. Pass `deterministic=False` only when throughput matters more than repeatability; hardware and third-party kernels can still introduce platform-level differences.

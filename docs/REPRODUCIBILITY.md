...
On 2026-08-28, release validation passed on Python 3.12.13 / macOS arm64 / CPU: all 8 tests with local data, integrity checking of the 131-file local package, parsing of all 14 original Python files, and all four CLI help entry points. The public manifest covers only the 15 original source/documentation files redistributed here. Without the dataset, the suite runs three synthetic model tests and skips five data-dependent checks. Hosted CI is not enabled; `ci-smoke.example.yml` is a template, not a completed CI run. The documented `--quick` training command also completed heuristic behavior cloning, two PPO epochs, checkpoint saving/loading, and final evaluation successfully. The two quick test episodes had completion ratio 1.0; this small functional run is not a benchmark. The [machine-readable record](release-validation.json) lists the dependency versions.
